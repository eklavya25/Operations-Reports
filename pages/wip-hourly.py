...
#     # iOS QC WIP
#     # ----------------------------
#     df_IOSQC_WIP = df1[df1.get('Pending For', '').isin(['IOS QC', 'Scan QC'])].copy()
...
#     df_3Shape = df_3Shape[~df_3Shape['Pending For'].isin(['IOS QC', 'Scan QC'])]
...
#     df_App["#Units"] = df_App["#Units"].apply(lambda x: 1 if x < 28 else 2)
...
#     df_EDDL = df_EDDL[~df_EDDL['Pending For'].isin(['IOS QC', 'Scan QC'])]
#     df_EDDL = df_EDDL[~df_EDDL.get('Skill Level', '').isin(['3Shape(7)', 'Exocad(7)'])]
...
# ----------------------------
# Safety wrapper to show full errors
# ----------------------------
def safe_run(func):
    try:
        func()
...
def main():
    ...
    if df.shape[0] < 8:
        st.error("Uploaded file doesn't look like the expected WIP export (too few rows).")
        return
    ...
    for col in ["Case Uploaded Time", "Restart Date"]:
